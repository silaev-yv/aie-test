...
def compute_quality_flags(
    ...
    high_cardinality_threshold = 0.9  # если уникальных значений > 90% от количества строк
    if df is not None and summary.n_rows > 0:
        for col_summary in summary.columns:
            if not col_summary.is_numeric and col_summary.unique > 0:
                cardinality_ratio = col_summary.unique / summary.n_rows
                if cardinality_ratio > high_cardinality_threshold:
                    high_cardinality_categoricals.append(col_summary.name)
    ...
    # 3. Проверка на много нулевых значений
    zero_heavy_columns: List[str] = []
    zero_threshold = 0.5  # если более 50% значений - нули
    if df is not None:
    ...
    # 4. Проверка на подозрительные дубликаты ID
    ...
    if df is not None:
        ...
        for col in id_like_columns:
            ...
            if total > 0 and unique < total:
                suspicious_id_duplicates.append(col)
